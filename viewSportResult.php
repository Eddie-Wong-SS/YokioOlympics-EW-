<?php
/**
Shows the results of sports search
 */
...
include ('Email.php');//Include emailing functionality
?>
...
<div class="container" align="center">
    ...
    if($_REQUEST['btnDelete'])
    {
        ...
        foreach($_POST as $key => $val)
        {
            ...
            {
                //Checks if the listed sport has a schedule
                $CheckSchedExists = "SELECT schedID FROM tblschedule WHERE sportID = '".$key."'";
                $CheckSchedExistsResult = mysqli_query($Link, $CheckSchedExists);

                if(mysqli_num_rows($CheckSchedExistsResult) > 0)
                {
                    $DelSchedSQL = "UPDATE tblschedule SET Status = 'I' WHERE sportID = '".$key."'"; //Sets associated schedule records to be inactive
                    $DelSchedSQLResult = mysqli_query($Link, $DelSchedSQL);
                    ...
                    if(mysqli_num_rows($CheckBookExistsResult) > 0)
                    {
                        ...
                        $Inform = "Dear ".$Booker.", <br/>  It is with utmost regret that we inform you that ".$Sport." has been taken down from the Yokio Olympics.
                            As such,all schedules for the sport has been cleared, and your booking for said schedules is invalid. 
                            We sincerely apologize for any inconvinience caused";
                        sendEmail("Booking Cancellation", $Inform, $RowBooks['email']);
                    }
                }

                $DelEmpSQL = "UPDATE tblsports SET Status = 'I' WHERE sportID = '".$key."'"; //Sets sport record to be inactive
                $DelEmpResult = mysqli_query($Link, $DelEmpSQL);
            }
        }
        if($DelEmpResult)
        {
            echo "<script>alert('Selected record(s) has been deleted');location='viewSportResult.php?page=1';</script>"; //Reloads the webpage after accounts have been deactivated
        }
    }
    else
    {
        $SQL = $_SESSION['SQL']; //Gets the SQL query from viewSport.php
            ...
            if(mysqli_num_rows($Result) > 0)
            {
                ...
                <form method="post" action="">
                    <?php if($maxPage > 1 && $page != $maxPage) //Checks if 'back' button should be shown
                        echo "<button><a style='font-size: 21px'  href='viewSportResult.php?page=".($page-1)."'>&#60;</a> </button>";
                    ...
                    <table align="center">
                        ...
                        <tr style="color: black">
                            ...
                            <th scope="col" style="width: 75%;">Sport Description</th>
                        ...
                        for($i = $minLim ; $i <= $maxLim; ++$i)
                        {
                            ...
                            echo "<td>".($i)."</td>";
                            if($_SESSION['AccType'] == "ADMIN" || $_SESSION['AccType'] == "STAFF")
                            echo "<td><input type=\"checkbox\" name=\"".$RowInfo['sportID']."\" id=\"Rec".($i)."\"></td>";

                            echo "<td>".$RowInfo['sportName']."</td>";
                            echo "<td style='text-align: center'>".$RowInfo['sportDescp']."</td>";
                            echo "<td style='text-align: center'><a class='hoverme' href=\"sportDetails.php?Id=".$RowInfo['sportID']."\">Sport Page</a></td>"; //Sends user to the staff editing page
                            echo "</tr>";
                        }
                        if($_SESSION['AccType'] == "ADMIN" || $_SESSION['AccType'] == "STAFF")
                        {
