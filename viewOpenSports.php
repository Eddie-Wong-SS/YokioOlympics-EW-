<?php
/**
Allows the viewing of unscheduled sports with filter options
 */
...
?>
<title>Filter Unscheduled Sports</title>
...
<?php if($_SESSION['AccType'] == "ADMIN" || $_SESSION['AccType'] == "STAFF") //Checks if user is admin/staff
{
    ...
    if($_REQUEST['btnSearch'])
    {
        ...
        $SQL = "SELECT * FROM tblsports WHERE tblsports.sportID NOT IN (SELECT sportID FROM tblschedule)";
        ...
        $_SESSION['SQL'] = $SQL;
        echo "<script>location='viewOpenSportsResult.php?page=1'</script>"; //Takes the user to the next page
    }
    ?>
</div>
</body>
